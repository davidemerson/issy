...
- **Undo/redo**: Each edit pushes an `UndoEntry` with position, deleted bytes (if any), and inserted length. Undo reverses the operation and pushes the inverse to the redo stack. Replace operations (which both delete and insert) produce a single combined entry. Consecutive word-character inserts within a 500 ms window at adjacent positions are folded into the previous entry via `tryCoalesceInsert` — typing "hello" is one undo step, not five. Whitespace chars, Enter, Tab, cursor movement, backspace, paste, and multi-cursor edits all end the coalesced run.
