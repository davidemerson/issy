...
- **Search**: smart-case (all-lowercase patterns match ASCII case-insensitively), implemented on `Buffer.find`. Incremental search re-anchors at the position where search mode was entered on every keystroke; Up/Down (or Ctrl+G) walk between matches, and match count / current index are recomputed per jump for the prompt's `current/total` counter.
