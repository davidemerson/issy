...
- **Atomic save**: resolves symlinks with readlink(2) so saving through a link rewrites the target (not the link), preserves the original permission bits with fchmod, writes to a uniquely named `.<name>.<hex>.issy-tmp` sibling opened with O_EXCL (never clobbers an existing file or follows a planted symlink), fsyncs the data — a failed fsync aborts the save instead of renaming a truncated temp into place — then renames and best-effort-fsyncs the parent directory. `load()` allocates the replacement backing array before freeing the old one, so allocation failure leaves the buffer intact.
...
- **Modes**: `normal`, `search`, `command`, `confirm`, `replace`, `help`. Each mode has its own key handler. `confirm` carries a `confirm_action` discriminator (`quit`, `new`, `open`, or `reload`) so the same confirm prompt dispatches to quit, new buffer, the open-file prompt, or a reload depending on which key triggered it. Confirm accepts Enter, Ctrl+Q, or Ctrl+W; Escape cancels; Ctrl+S saves the buffer and drops the pending action (re-issuing it then proceeds without a prompt, since the buffer is clean). Ctrl+S is never silently swallowed in any mode: search, replace, and the help overlay save and return to normal, and in the Save-As prompt it submits.
...
- **Swap-file autosave** (on by default): while the buffer is dirty, the main loop's 1/sec tick writes it to a sibling `.<name>.swp` (throttled to every 2s), removed on save, reload, new buffer, file switch, and clean quit — but only when this session wrote it. A foreign swap (crash recovery from an earlier session) is reported on open (not auto-loaded) and survives a clean quit; it is only replaced once this session's own autosave supersedes it.
...
- **Multiple cursors**: `Ctrl+D` puts every cursor at the start of the word under the primary and adds a cursor at the next occurrence, arming a rename: the first character typed replaces the word at every cursor (one undo group), and further typing inserts normally. Any other key disarms the rename; editing operations apply to all cursors; overlapping cursors merge; Escape clears extras. Paste inserts at every cursor.
- **Bracketed paste**: On init the terminal enables DECSET 2004. term.zig captures the entire paste body between `ESC[200~` and `ESC[201~` into a `.paste` event (chunked above 64 KB, capped at 16 MB per session so a lost terminator can't swallow keystrokes forever). `handleKey` intercepts `.paste` before the per-mode dispatch and inserts it verbatim as one undo step — auto-indent and tab expansion never run on pasted content, a newline inside a paste can't confirm a prompt, and control bytes are stripped when the target is a single-line prompt field.
- **System clipboard (OSC 52)**: `copySelection` pushes the copied bytes out to the terminal's system clipboard via `term.writeOsc52Clipboard`, which base64-encodes in 768-byte chunks (a multiple of 3, so no padding appears mid-stream) and wraps the result in `ESC]52;c;<b64>\a`. Capped at 100 KB of raw data; longer selections only go to the internal clipboard. Paste direction (OS → issy): `Ctrl+V` and middle-click emit an OSC 52 read query and wait briefly for the terminal's reply, falling back to issy's internal clipboard when the terminal doesn't answer (gated by the `system_clipboard` config key); the terminal's own paste keys arrive as bracketed paste.
...
- **Notify** (`startupCheck`, `readCachedState`, `shouldNotify`): On editor startup, reads `~/.cache/issy/commit.txt` — `<sha> <epoch>` — and compares against `build_info.commit_sha`/`commit_epoch`. The release must be *strictly newer*, not merely different: comparing SHAs for inequality advertised a downgrade after any manual upgrade, since the installer never refreshed the cache. When either epoch is unknown (a no-git tarball build, or a cache written by an older version) it falls back to inequality — notification fails open, because a wrong status line is cheap. Dev builds short-circuit the check entirely.
...
- **Apply** (`apply`, `canAutoApply`, `verifyStagedBinary`, `applyOrderingOk`, `writeResumeFile`): In the main loop, when all gates are satisfied (a staged binary exists, `autoupdate` is on, buffer is clean, `argv0` is writable, the editor has been idle for 60 seconds), the editor re-verifies the staged binary against the cached signed manifest — it may have sat in the user-writable cache for days — deleting it on any mismatch. Verification also requires the manifest's signed commit and epoch headers to name a release *strictly newer* than the running build: signature-and-hash alone is satisfied by an authentic OLDER release, so without this a staged N+1 would silently reinstall itself over a hand-installed N+2. Unlike notification, this fails closed on any unknown. The swap falls back to copy-then-rename inside the target directory when the cache is on another filesystem, and the re-exec preserves `--config`/`--theme`/`--font`/`--no-config`. It then writes a one-shot resume record, snapshots the running binary to `issy.prev` (recording its SHA-256 in `issy.prev.sha256`), atomically renames the staged binary over its own executable, tears down the terminal, and `execve()`s the new binary with `--resume <path>`. Termios state lives on the tty device, not the file descriptor, so the terminal survives `execve` cleanly — the user sees one re-render.
...
- **Rollback** (`rollback`): `issy --rollback` runs before any TUI init: it verifies `issy.prev` against the checksum recorded at apply time, renames it back over `argv0`, deletes the now-orphaned checksum, and exits. One-shot, atomic. An *absent* checksum still skips verification (pre-checksum snapshots), but one that is present and unparseable is treated as corruption and refused — truncating that file is easier than forging a matching hash, so failing open there defeated the guard's whole purpose.
...
The worker declines to download at all when the binary isn't writable (a root-owned install is genuinely notify-only, not a download that can never be installed) or when a staged binary already matches the advertised release. A failed apply demotes `UpdateState.status` to `.error_state` so it isn't retried every 60 seconds for the rest of the session, and `startupCheck` prunes stale `*.tmp.*` and `resume.*.txt` litter that a SIGALRM-killed worker leaves behind.

**Testing.** `build.zig` generates a gitignored `src/update_config.zig` from `-Dupdate-base-url`, `-Dupdate-pubkey` and `-Dupdate-min-idle-ms`, letting `tests/t25_update.exp` drive the whole chain against a loopback fixture server signed with a throwaway key. A plain `zig build` rewrites that file with all-null values on every configure, so the overrides fold away at comptime and cannot reach a release binary; a malformed value is a hard build failure; and an override build stamps `test-update` into `--version`, which the suite asserts is absent from `zig-out/bin/issy`.
